...
#### 基础环境搭建
- Docker Compose 配置完成，包含 MySQL 8.0 和 MinIO 服务
- 数据库初始化脚本编写完成（包含 6 张表：sku_category, sku, sku_price_log, customer, order, order_item, user）
- MinIO 对象存储配置完成，支持图片上传和公开访问

#### 数据导入脚本
- 开发完整的 SKU 导入脚本 (`scripts/import_sku.py`)
- 支持 Excel 多 sheet 读取和数据合并
- 实现产品名称去重和 SKU 自动生成逻辑
- 支持从 Excel 中提取嵌入图片并上传到 MinIO
- 处理多种 Excel 格式（标准和非标准格式）
- 实现产品类目字段映射和分类自动识别
...
|---------|------|------|
| SKU | 195 条 | 包含 6 个分类 |
| SKU 分类 | 6 个 | 食品、纸品、个护、百货、电器、服饰床品 |
| 客户 | 2 个 | 方舟、SE8加拿大 |
| 产品图片 | 195 张 | 100% 上传到 MinIO |
| 价格变更记录 | 4 条 | 测试数据 |
...
**交付物**:
- [x] `backend/main.py` - FastAPI 应用入口文件
- [x] `backend/requirements.txt` - Python 依赖包列表
- [x] `backend/app/core/config.py` - 配置文件（环境变量管理）
...
**任务清单**:
- [x] 创建 `scripts/` 目录结构
- [x] 开发 Excel 解析脚本（使用 openpyxl）
- [x] 开发图片上传脚本（上传到 MinIO）
- [x] 开发数据清洗脚本（处理字段映射）
- [x] 开发数据库导入脚本（使用 SQLAlchemy）
- [x] 实现整箱数量特殊处理逻辑
- [x] 实现产品名称去重和 SKU 自动生成
- [x] 实现数据验证和错误处理
...
**任务清单**:
- [x] 确认本期功能范围（SKU管理、订单管理、数据导入、报表）
- [x] 梳理数据表关系（customer、sku_category、sku、order、order_item）
- [x] 梳理业务流程（SKU 创建、订单录入、Excel 导入、对账单生成）
- [x] 梳理 API 接口设计
- [x] 确认数据模型（Pydantic Models）
- [x] 确认数据库迁移策略（Alembic）
...
- [x] `docker-compose.yml` - Docker 编排文件
- [x] `scripts/` - 数据导入脚本目录
  - [x] `import_orders.py` (已删除)
  - [x] `import_sku.py`
  - [x] `minio_client.py`
- [x] 数据导入验证报告
...
| 里程碑 | 完成标志 | 预计时间 | 状态 |
|-------|---------|---------|------|
| M1: 基础环境搭建完成 | Docker 环境启动成功，health 接口正常 | 第3天 | ✅ 已完成 |
| M2: 数据导入完成 | Excel 数据成功导入 MySQL 和 MinIO | 第3天 | ✅ 已完成 |
| M3: 后端 API 开发完成 | 所有 API 接口开发完成并通过测试 | 第18天 | 🔄 进行中 |
| M4: 前端页面开发完成 | 所有功能页面开发完成 | 第33天 | ⏳ 待开始 |
| M5: 集成测试通过 | 前后端联调成功，功能验收通过 | 第33天 | ⏳ 待开始 |
